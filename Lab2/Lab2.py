...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.zeros(shape=int(len(self.nodes[0])), dtype=np.double)
        self.velocity = np.zeros(shape=(3, int(len(self.nodes[0]))), dtype=np.double)
        ...
        for i in range(int(len(self.velocity[0]))):
            if self.nodes[1][i] > 29:
                self.moving_nodes.append(i)

        for i in range(int(len(self.velocity[0]))):
            if self.nodes[0][i] < -117 or self.nodes[0][i] > -40 and i not in self.moving_nodes:
                self.wings_nodes.append(i)
    ...
    def move(self, tau, step):
        ...
        for i in range (int(len((self.nodes[0])))):
            if i in self.moving_nodes:
                self.smth[i] = 5 * np.sin(self.nodes[0][i] * 0.1 + tau * step * 5)
            elif i in self.wings_nodes:
                self.smth[i] = 5 * np.sin((min(abs(self.nodes[0][i] + 117), abs(self.nodes[0][i] + 40)) ** 2)*0.01 - tau * step*5)
            else:
                self.smth[i] = 5*np.sin((((self.nodes[0][i] + 77) ** 2 + (self.nodes[1][i] - 16) ** 2 + (self.nodes[2][i]) ** 2)) ** 0.5) + 4
        for i in self.moving_nodes:
            self.velocity[2][i] = self.smth[i]
        for i in self.wings_nodes:
            self.velocity[1][i] = self.smth[i]
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("VDV_moves/vdv-step-" + str(snap_number) + ".vtu")
        writer.Write()
